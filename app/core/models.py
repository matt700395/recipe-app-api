...
class UserManager(BaseUserManager):
    ...
    def create_user(self, email, password=None, **extra_fileds):
        ...
        user = self.model(email=self.normalize_email(email), **extra_fileds) #user라고하는 변수로 email을 model에 받아 오겠다는 뜻
        #self.normalize_email() 은 BaseUser Manager에서 제공하는 함수
...
class User(AbstractBaseUser, PermissionsMixin): #장고 사용자 모델을 사용하지만, 그 위에 빌드하고 사용자정으를 할 수 있다
    ...
    email = models.EmailField(max_length=255, unique=True, default='matt7003@korea.ac.kr') #unique=Ture면 한명당 email 하나만 만들고 사용가능
